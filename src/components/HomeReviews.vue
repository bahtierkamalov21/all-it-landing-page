            ...
            v-card(class="card py-4 rounded-lg" elevation="0")
              ...
              div(class="swiper-reviews")
                div(class="swiper-wrapper")
                  div(class="swiper-slide" v-for="(item, index) in $store.state.reviews" :key="index")
                    div(class="d-flex flex-wrap reviews-top align-center justify-space-between")
                      v-list
                        v-list-item
                          v-list-item-avatar
                            v-img(v-if="item.user.avatar" :src="item.user.avatar")
                            v-icon(v-else style="font-size: 48px;") mdi-account-circle
                          v-list-item-content(class="pl-0")
                            v-list-item-title {{ item.user.first_name ? item.user.first_name : item.user.username }}
                            v-list-item-subtitle {{ item.user.last_name }}
                      div(class="d-flex align-center pr-6")
                        v-icon(color="yellow" v-for="(rating, index) in item.rating" :key="index") mdi-star
